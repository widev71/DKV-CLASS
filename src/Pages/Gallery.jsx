...
const Carousel = () => {
	...
	// Function to fetch image list from Firebase Storage
	const fetchImagesFromFirebase = async () => {
		try {
			const storage = getStorage(); // Get Firebase Storage reference
			const storageRef = ref(storage, "GambarAman/"); // Use ref with storage

			const imagesList = await listAll(storageRef); // Use listAll to get the list of images

			const imageURLs = await Promise.all(
				imagesList.items.map(async (item) => {
					const url = await getDownloadURL(item); // Use getDownloadURL to get the image URL
					return url;
				}),
			);
